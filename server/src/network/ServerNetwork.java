...
public class ServerNetwork extends Thread {
	...
	/**
	 * Use the UDP class to intialise the socket. The address and port can be
	 * specified in this version of the method.
	 * 
	 ...
	 * 
	 * @return An int giving the port number that the server application is using on
	 *         the local host.
	 */
	...
	/**
	 * Method to pass a reference to the model to the network class.
	 * 
	...
	/**
	 * Method to pass a reference to the console to the network class.
	 * 
	...
	/**
	 * Method to pass a reference to the comm to the network class.
	 * 
	...
	/**
	 * Method to pass a reference to the connections to the network class.
	 * 
	...
	/**
	 * This thread's run method. Loops constantly attempting to receive packets and
	 * either register or deregister users with this information.
	 * <p>
	 * Obtains the client address and port from this information. After removing the
	 * packet tag, this method deduces what kind of request the packet is for.
	 * <p>
	 * Whether the packet is telling the server about a registration or
	 * deregistration, the server will then go through the peer list and send an
	 * updated peer list to every peer.
	 */
	public void run() {
		try {
			console.printConnectionDetails(getServerAddress(), getServerPort());
		...
		/*
		 * if(!getPresState()) { console.retrievePresDetails(); }
		 */
	...
	public void netRoutine() {
		...
		if (packetTag.equalsIgnoreCase("REGISTER")) {
			...
			String clientPublicKey = packetContents.get(1);
			register(clientUsername, clientAddress, clientPort, clientPublicKey);
			...
			console.lifeVerified(clientUsername, clientAddress, clientPort);
		}
		...
		file.storeMessage(time, packetTag, method, encryption, clientUsername, clientAddress,
				Integer.toString(clientPort), "SERVER", serverAddress, Integer.toString(serverPort), packetContents,
				Integer.toString(size), transTime);
		forward(time, packetTag, method, encryption, clientUsername, clientAddress, Integer.toString(clientPort),
	...
	/**
	 * Cycle through the list of peers and send an updated version of the peer list
	 * to each one.
	 */
	public void distributePeerLists() {
		...
		for (Peer peer : model.getPeers()) {
			...
			length = addData("PEER_LIST", packetData, peerAddress, peerPort);
	 ...
	 * @return
	 */
	public boolean register(String clientUsername, String clientAddress, int clientPort, String publicKey) {
		Peer newPeer;
		...
		if (model.usernameAvailable(clientUsername)) {
			length = addData("REGISTRATION_SUCCESS", packetData, clientAddress, clientPort);
			newPeer = new Peer(clientUsername, clientAddress, clientPort, publicKey);
			model.registerPeer(newPeer);
			console.registerPeer(clientUsername, clientAddress, clientPort);
			distributePeerLists();
		} else {
			packetData.add("Username already in use.");
			length = addData("ERROR", packetData, clientAddress, clientPort);
		}
		return true;
	}
	 ...
	 * @param clientPort     The port of the client.
	 */
	public void deregister(String clientUsername, String clientAddress, int clientPort) {
	 ...
	 */
	public void forward(String time, String tag, String method, String encryption, String clientUsername,
			String clientAddress, String clientPort, List<String> contents, String size, String transTime) {
		if (!getPresState()) {
			return;
		}
		String dateString;
	 ...
	 * @param clientPort
	 */
	public int addData(String packetTag, List<String> packetData, String clientAddress, int clientPort) {
		int returnVal = 0;
		String dateString;
		...
		packetData.add(encryption);
		try {
			returnVal = comm.sendPacket(clientAddress, clientPort, packetTag, packetData);
		} catch (IOException ioException) {
			console.printError("An error occured while creating the " + "packet.");
		}
		file.storeMessage(dateString, packetTag, method, encryption, "SERVER", serverAddress,
				Integer.toString(serverPort), model.findUsername(clientAddress, clientPort), clientAddress,
