...
/**
 * This class is designed to deal with the client's registration with the server
 * well as identify incoming packets from other peers.
 * <p>
 * When receiving messages from other peers, this class also triggers events in
 * the view to ensure that the GUI changes and opens tabs appropriately (and
 * also the console).
 * 
 * @author Alex
 * @version 0.2
...
public class ClientModel implements Runnable {
	...
	/**
	 * Set the username. Also calls the registerClent() method which will send a
	 * registration packet to the server.
	 * 
	 ...
	 */
	public void initSession(String username, String serverIP, String serverPort) {
		this.username = username;
		this.serverIP = serverIP;
		this.serverPort = serverPort;
	 ...
	 * 
	 * @return Returns T for registration success and F for registration failure.
	 */
	...
	/**
	 * Sets the serverRegistered boolean to either T or F depending on parameter.
	 * 
	...
	/**
	 * Attempt to register the client by causing the ClientNetwork Object to contact
	 * the server.
	 */
	...
	/**
	 * Deregister the client by causing the ClientNetwork Object to contact the
	 * server.
	 */
	 ...
	 * Receive a message.
	 * <p>
	 * This involves cycling through the list of peers and checking the incoming IP
	 * address with those listed to determine who is the sender.
	 * 
	...
	public void receiveMessage(String srcAddress, int srcPort, String time, String message) {
		...
		for (Peer peer : peerList.values()) {
			if ((peer.getAddress().equalsIgnoreCase(srcAddress)) && (peer.getPort() == srcPort)) {
				System.out.println(
						"Received message for user " + peer.getUsername() + " with public key " + peer.getPublicKey());
	 ...
	 * Object is then contacted to deal with the specifics of sending the actual
	 * packet.
	 * 
	 ...
	 * <p>
	 * The peer information is taken out of the list and stored before a method is
	 * called from the ClientView to reflect these changes in the GUI.
	 * 
	...
	public void updatePeerList(List<String> list) {
		...
		for (String peerString : list) {
			...
			String peerPublickey = peerDetails[3];

			peerList.put(peerAddress + ":" + peerPort, new Peer(peerUsername, peerAddress, peerPort, peerPublickey));
		}
		console.newPeerList();
	 ...
	 * username and port.
	 * <p>
	 * If the address/port combination is not found in the peer list, this method
	 * will return the string "SERVER".
	 * 
	 * @param address
	 * @param port
	 * @return
	...
	public String findUsername(String address, int port) {
		...
		for (Peer peer : peerList.values()) {
			if (peer.getAddress().equalsIgnoreCase(address) && peer.getPort() == port) {
	...
	/**
	 * Trigger changes in the view to reflect changes to filenames, lists or the
	 * finger table.
	 */
	 ...
	 * @param selectedFile The file object of the desired local file to be
	 *                     transferred.
	 */
	...
	/**
	 * Allow the user the login to the model's initialised ClientLogin object.
	 */
	...
	/**
	 * Set the boolean that keeps track of whether a duplicate username has been
	 * provided. Receiving an "ERROR" packet fro the server when logging in will
	 * trigger this method.
	 * 
	 * @param status True for duplicate, false for unique or not logged in.
	 */
	...
	/**
	 * Retrieve the duplicate status of this model to determine whether or not.
	 * 
	 * @return True for duplicate, false for unique or not logged in.
	 */
