...
public class ClientNetwork implements Runnable {
	...
	/**
	 * Set the method string for all packets forwarded to the presentation server.
	 * 
	 * @param method The method string. (e.g. UDP or TCP)
	 */
	...
	/**
	 * Set the presState parameter to determine whether or not the presentation
	 * details have been set for this client.
	 * 
	 * @param state
	 */
	...
	/**
	 * Get the presState parameter. This determines whether or not the presentation
	 * details have been set for this client.
	 * 
	 * @return
	 */
	...
	/**
	 * This is the network's run method that attempts to retrieve a packet and
	 * determine what course of action it should take.
	 * <p>
	...
	public void run() {
		...
		/*
		 * if(!getPresState()) { console.request(); } else { console.obtainUsername(); }
		 */
		...
		while (isRunning()) {
			if ((!runningThread.isInterrupted()) || (dht.isDhtRegistered()) || (model.getServerRegistered())) {
				try {
					netRoutine();
	...
	/**
	 * Method for receiving packet and determining what course of action to take
	 * depending on the packet tag.
	 * 
	 * @throws InterruptedException This means that the socket has timed out. It
	 *                              returns null in this case and throws an
	...
	private void netRoutine() throws InterruptedException {
		...
		try {
			...
			if (packetTag.equalsIgnoreCase("ERROR")) {
				...
				model.receiveMessage(sentAddress, sentPort, packetContents.get(0), packetContents.get(1));
				...
				length = addData("LIFE_CONFIRM", packetData, sentAddress, sentPort);
				...
				List<String> packetData = new ArrayList<String>();
				addData("DHT_PRED_CONFIRM", packetData, sentAddress, sentPort);
			...
			file.storeMessage(time, packetTag, method, encryption, model.findUsername(sentAddress, sentPort),
					sentAddress, Integer.toString(sentPort), model.getUsername(), clientAddress,
					Integer.toString(clientPort), fileDupe, Integer.toString(size), transTime);

			forward(time, packetTag, method, encryption, model.findUsername(sentAddress, sentPort), sentAddress,
					Integer.toString(sentPort), forwardDupe, Integer.toString(size), transTime);
			try {
			...
			console.printError("An IO error occured while " + "recieving a packet.");
		}
	}
	 ...
	 */
	public void registerClient(String username, String serverAddress, String serverPort, String publickey) {
		List<String> packetData = new ArrayList<String>();
		int length;

		packetData.add(username);
		packetData.add(publickey);
		length = addData("REGISTER", packetData, serverAddress, Integer.parseInt(serverPort));
	}

	/**
	 * Send a deregistration notice from the user to the currently connected server.
	 * 
	 * @param username      The username of the client.
	 * @param serverAddress The IP address of the current server.
	 * @param serverPort    The
	 */
	public void deregisterClient(String username, String serverAddress, String serverPort) {
		List<String> packetData = new ArrayList<String>();
		int length;

		packetData.add(username);
		length = addData("DEREGISTER", packetData, serverAddress, Integer.parseInt(serverPort));
	}
	...
	public void sendMessage(String message, String destAddress, int destPort) {
		...
		length = addData("MESSAGE", packetData, destAddress, destPort);
	}
	 ...
	 */
	public void forward(String time, String tag, String method, String encryption, String destUsername,
			String destAddress, String destPort, List<String> contents, String size, String transTime) {
		if (!getPresState()) {
			return;
		}
		String dateString;
		...
		try {
			comm.sendPacket(presAddress, Integer.parseInt(presPort), tag, packetData);
		} catch (IOException ioException) {
			console.printError("An error occured while creating the " + "presentation packet.");
		}
	}
	...
	public void notifyPresentationServer(String header, List<String> contents) {
		...
		try {
			...
			comm.sendPacket(presAddress, Integer.parseInt(presPort), header, contents);
		} catch (IOException ioException) {
			console.printError("An error occured while creating the " + "presentation packet.");
		}
	}
	...
	/**
	 * Function to add the extra information to packet so data can be tracked.
	 * 
	 * @param packetTag
	 * @param packetData
	 * @param clientAddress
	 * @param clientPort
	 */
	public int addData(String packetTag, List<String> packetData, String destAddress, int destPort) {
		int returnVal = 0;
		String dateString;
		...
		// Add encryption.
		packetData.add(encryption);
		System.out.println("Sending " + packetTag + packetData + " to " + destAddress + ":" + destPort);
		try {
			returnVal = comm.sendPacket(destAddress, destPort, packetTag, packetData);
		} catch (IOException ioException) {
			console.printError("An error occured while creating the " + "packet.");
		}
		try {
			file.storeMessage(dateString, packetTag, method, encryption, model.getUsername(), comm.getClientAddress(),
					Integer.toString(comm.getClientPort()), model.findUsername(destAddress, destPort), destAddress,
	...
	/**
	 * Call for a refresh in the GUI in order to display and changes that have been
	 * made to reflect new DHT information.
	 */
	...
	/**
	 * Set the ID of the last node that a DHT_DOWN packet has been sent to.
	 * 
	...
	/**
	 * Set the ID of the last node that a DHT_UP packet has been sent to.
	 * 
	 * @param lastId ID of the receiving node.
	 */
	 ...
	 * 
	 * @param dhtStatus True will make the DHT interface appear whilst false will
	 *                  make the centralised interface appear.
	 */
	...
	public void receiveFile(String fileName, String address, int filePort, int port) {
		Runnable r = new FileReceiver(fileName, address, filePort, null, model.getView(),
				model.findUsername(address, port));
		new Thread(r).start();
	}
	...
	public int sendFile(File newFile, String destAddress, int destPort) {
		FileSender r = new FileSender(newFile, null, model.getView(), model.findUsername(destAddress, destPort));
	...
	public void sendFileRequest(String destAddress, int destPort, File selectedFile) {
		...
		addData("FILE_SEND", packetData, destAddress, destPort);
	}

	/**
	 * Method that should be called whenever contact has been made with the server
	 * or DHT node that this peer is joining to.
	 * <p>
